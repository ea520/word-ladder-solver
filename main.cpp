...
int main(int argc, char *argv[])
{
    ...
    program.add_argument("-l", "--word-list")
        .help("Path to the list of keywords")
        .default_value(std::string("word-list.txt"));

    program.add_argument("-s", "--start")
        .help("The starting word in the game (must be 4 letters)")
        .required()
        .default_value(std::string("SLOW"));

    program.add_argument("-e", "--end")
        .help("The ending word in the game (must be 4 letters)")
        .required()
    ...
    const std::string *start = &*std::find(words.begin(), words.end(), to_upper(program.get<std::string>("-e"))); // swap end and start
    const std::string *end = &*std::find(words.begin(), words.end(), to_upper(program.get<std::string>("-s")));
    if (end == &*words.end())
    {
        std::cerr << "The starting word is not in the worrd list\n";
        return -1;
    }
    if (start == &*words.end())
    {
        std::cerr << "The ending word is not in the worrd list\n";
        return -1;
    }
    ...
    std::cout << "Duration: " << std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count() << "ms" << std::endl;
    size_t visited_count = std::count_if(cache.begin(), cache.end(), [](std::pair<const std::string *, size_t> s)
                                         { return s.second != SIZE_MAX; });
    std::cout << "Number of nodes seen " << visited_count << std::endl;
    print_tree(root);
}
